...
// =============================================================
// ASSISTANCE STATE — localStorage
// =============================================================
const ASSIST_KEY = 'bmis_assistance_ids';

function getCheckedIds() {
    try { return JSON.parse(localStorage.getItem(ASSIST_KEY) || '[]'); }
    catch (e) { return []; }
}
function saveCheckedIds(ids) {
    localStorage.setItem(ASSIST_KEY, JSON.stringify(ids));
}
function isAssisted(id) {
    return getCheckedIds().includes(parseInt(id));
}
...
$(document).ready(function () {
    ...
    householdsTable = $('#householdsTable').DataTable({
        ...
        ajax: {
            ...
            error: function (xhr) { console.error('DT error:', xhr.status); showToast('error', 'Failed to load records.'); }
        },
        ...
        drawCallback: function () {
            $('#householdsTable tbody tr').each(function (i) {
                ...
                setTimeout(() => $tr.css({ transition: 'opacity .2s, transform .2s', opacity: 1, transform: 'translateY(0)' }), i * 30);
            });
        }
    });
...
$(document).on('click', '.view-household', function (e) {
    ...
    $.ajax({
        url: baseUrl + 'households/get/' + id, type: 'GET', dataType: 'json',
        success: function (res) {
            ...
            if (res.status === 'success') {
                ...
                $('#householdDetailBody').html('<div class="text-center py-4 text-danger"><i class="fas fa-exclamation-circle fa-2x mb-2 d-block"></i>' + (res.message || 'Could not load.') + '</div>');
            }
        },
        error: function () {
            $('#householdDetailBody').html('<div class="text-center py-4 text-danger"><i class="fas fa-exclamation-circle fa-2x mb-2 d-block"></i>Server error.</div>');
        }
    });
});

function renderHouseholdDetail(h) {
    const statusMap = { 'Active': ['badge-active','fa-check-circle'], 'Relocated': ['badge-relocated','fa-truck-moving'], 'Archived': ['badge-archived','fa-archive'] };
    ...
    $('#householdDetailBody').html(`
        <div class="detail-hero">
            ...
            <span class="status-badge ${sCls}" style="display:inline-flex;font-size:.8rem;"><i class="fas ${sIcon}"></i>&nbsp;${h.status || '—'}</span>
            ${aided ? '<div class="mt-2"><span class="assist-needs-badge"><i class="fas fa-heart mr-1"></i>Marked for Assistance</span></div>' : ''}
        </div>
        <div class="detail-rows">
            <div class="detail-row"><div class="detail-row-icon"><i class="fas fa-map-marker-alt"></i></div><div><div class="detail-row-label">Full Address</div><div class="detail-row-value">${fullAddr||'—'}</div></div></div>
            <div class="detail-row"><div class="detail-row-icon"><i class="fas fa-th-large"></i></div><div><div class="detail-row-label">Purok</div><div class="detail-row-value">${h.purok?purokBadge(h.purok):'—'}</div></div></div>
            <div class="detail-row"><div class="detail-row-icon"><i class="fas fa-users"></i></div><div><div class="detail-row-label">Total Members</div><div class="detail-row-value">${membersBadge(h.total_members)}</div></div></div>
            <div class="detail-row"><div class="detail-row-icon"><i class="fas fa-calendar-alt"></i></div><div><div class="detail-row-label">Registered</div><div class="detail-row-value">${date}</div></div></div>
        </div>`);
}
...
function fetchHouseholdStats() {
    $.ajax({
        url: baseUrl + 'households/fetchStats', type: 'GET', dataType: 'json',
        success: function (res) {
            if (res.status === 'success') {
                const d = res.data;
...
function animateStat(id, target) {
    ...
    let cur = 0; el.innerHTML = '0';
    const t = setInterval(() => { cur += step; if (cur >= target) { cur = target; clearInterval(t); } el.textContent = cur; }, 16);
}
function setStatError() {
    ['statTotal','statActive','statNew','statPriority'].forEach(id => { const el = document.getElementById(id); if (el) el.textContent='—'; });
}
...
function openAssistanceModal() {
    $('#assistListWrapper').html('<div class="text-center py-5"><div class="spinner-border text-danger"></div><p class="mt-2 text-muted small">Loading…</p></div>');
    ...
    $.ajax({
        url: baseUrl + 'households/fetchAssistancePriority', type: 'GET', dataType: 'json',
        success: function (res) {
            updateCSRF(res);
            if (res.status === 'success') {
                ...
                $('#assistListWrapper').html('<div class="assist-empty"><i class="fas fa-exclamation-circle d-block"></i><p>Could not load.</p></div>');
            }
        },
        error: function () {
            $('#assistListWrapper').html('<div class="assist-empty"><i class="fas fa-exclamation-circle d-block"></i><p>Server error.</p></div>');
        }
    });
}
...
function renderAssistanceList(data, q) {
    ...
    const filtered = q ? data.filter(h => [h.head_name,h.address_line1,h.purok,h.city_municipality].join(' ').toLowerCase().includes(q.toLowerCase())) : data;

    $('#assistSummaryText').html(`<i class="fas fa-list mr-1"></i><strong>${data.length}</strong> active &nbsp;|&nbsp; Showing <strong>${filtered.length}</strong>`);
    $('#assistCheckedCount').html(ids.length > 0 ? `<span class="badge badge-danger px-2 py-1"><i class="fas fa-heart mr-1"></i>${ids.length} marked</span>` : '');

    if (!filtered.length) { $('#assistListWrapper').html('<div class="assist-empty"><i class="fas fa-search d-block"></i><p>No households found.</p></div>'); return; }
    ...
    filtered.forEach((h, i) => {
        ...
        <div class="assist-card ${chk?'is-checked':''}" data-id="${h.id}">
            ...
            <div class="assist-card-body">
                <div class="assist-card-name"><i class="fas fa-home mr-1 text-muted" style="font-size:.8rem;"></i>${h.head_name||'—'}${chk?'<span class="assist-needs-badge ml-2"><i class="fas fa-heart"></i> Needs Assistance</span>':''}</div>
                <div class="assist-card-addr"><i class="fas fa-map-marker-alt mr-1"></i>${h.address_line1||''}${h.city_municipality?', '+h.city_municipality:''}</div>
                <div class="assist-card-meta">${purokBadge(h.purok)}<span class="badge-members"><i class="fas fa-users mr-1"></i>${h.total_members||1}</span><span class="status-badge badge-active" style="font-size:.7rem;padding:2px 8px;"><i class="fas fa-check-circle"></i> Active</span></div>
            </div>
        </div>`;
    });
...
$(document).on('change', '.assist-checkbox', function () {
    ...
    const ids = getCheckedIds();
    $(this).is(':checked') ? (!ids.includes(id) && ids.push(id)) : ids.splice(ids.indexOf(id),1);
    saveCheckedIds(ids);

    const $card = $(this).closest('.assist-card');
    if ($(this).is(':checked')) {
        $card.addClass('is-checked');
        $card.find('.assist-card-name .assist-needs-badge').remove();
    ...
    $('#assistCheckedCount').html(ids.length > 0 ? `<span class="badge badge-danger px-2 py-1"><i class="fas fa-heart mr-1"></i>${ids.length} marked</span>` : '');
    updateAssistStatCard();
    // Sync table row buttons
    householdsTable.rows().every(function () {
        const row = this.data(); if (!row) return;
        const $btn = $(householdsTable.row(this.index()).node()).find('.toggle-assist');
        const a = ids.includes(parseInt(row.id));
        $btn.removeClass('btn-act-assist-on btn-act-assist-off').addClass(a?'btn-act-assist-on':'btn-act-assist-off');
        $btn.find('i').removeClass('fa-heart fa-hands-helping').addClass(a?'fa-heart':'fa-hands-helping');
    });
});
...
$('#assistSearch').on('keyup input', function () { renderAssistanceList(assistAllData, $(this).val().trim()); });
...
$('#addHouseholdForm').on('submit', function (e) {
    ...
    $.ajax({
        url: baseUrl+'households/save', type:'POST',
        data: $(this).serialize()+'&csrf_test_name='+getCsrf(), dataType:'json',
        success: function (res) {
            updateCSRF(res);
            if (res.status==='success') {
                ...
                householdsTable.ajax.reload(null,false);
                fetchHouseholdStats();
            } else { showAlert('addAlertBox','danger',res.message); }
        },
...
function loadEditModal(id) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            } else { showToast('error', res.message||'Could not load.'); }
        },
...
$('#editHouseholdForm').on('submit', function (e) {
    ...
    $.ajax({
        url: baseUrl+'households/update', type:'POST',
        data: $(this).serialize()+'&csrf_test_name='+getCsrf(), dataType:'json',
        success: function (res) {
            updateCSRF(res);
            if (res.status==='success') {
                ...
                householdsTable.ajax.reload(null,false);
                fetchHouseholdStats();
            } else { showAlert('editAlertBox','danger',res.message); }
        },
        error: function () { showToast('error','Server error.'); },
        complete: function () { $btn.prop('disabled',false).html('<i class="fas fa-save mr-1"></i>Update Household'); }
    });
});
...
$(document).on('click', '.delete-household', function () {
    ...
    confirmFn.then(function (confirmed) {
        ...
        setTimeout(function () {
            $.ajax({
                url: baseUrl+'households/delete/'+id, type:'POST',
                data: { csrf_test_name: getCsrf() }, dataType:'json',
                success: function (res) {
                    updateCSRF(res);
                    if (res.status==='success') {
...
function showAlert(boxId, type, message) {
    $('#'+boxId).html(`<div class="alert alert-${type} alert-dismissible py-2 px-3 mb-2"><i class="fas fa-exclamation-circle mr-1"></i>${message}<button type="button" class="close py-2" data-dismiss="alert">&times;</button></div>`);
}

$('#addHouseholdModal').on('hidden.bs.modal', function () { $('#addHouseholdForm')[0].reset(); $('#addAlertBox').html(''); });
$('#editHouseholdModal').on('hidden.bs.modal', function () { $('#editAlertBox').html(''); });